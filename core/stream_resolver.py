...
#
# Раньше здесь были только -reconnect/-reconnect_streamed/-reconnect_delay_max —
# этого хватает только на явный обрыв TCP-соединения. Пойманный вживую
# случай (плановая запись 'Звезда' оборвалась на середине, судя по логам —
# именно так, без явной ошибки) — это два ДРУГИХ отказа, которые старый
# набор не ловит:
#   -reconnect_on_network_error — обрыв на уровне TCP/TLS при попытке
#     переподключиться (не просто "поток кончился", а сама сеть моргнула);
#   -rw_timeout — соединение осталось открытым, но сервер молчит (не шлёт
#     данные и не закрывает сокет) — без явной ошибки ffmpeg просто ждал бы
#     вечно; тут заставляем его сдаться через 20с тишины, что уже ловится
#     как сетевая ошибка и уходит в reconnect выше.
# -reconnect_delay_max подняли с 5 до 10 (сам по себе — это потолок паузы
# ПЕРЕД одной попыткой, не общий бюджет) и добавили -reconnect_delay_total_max
# 600 — сколько секунд суммарно можно потратить на попытки, прежде чем
# ffmpeg сдастся окончательно; раньше действовал дефолт в 256с, для
# нестабильной сети (а не разового обрыва) этого может не хватить.
#
# -reconnect_at_eof НАРОЧНО убран (был тут при добавлении фикса для 'Звезда'
# выше) — напрямую воспроизвёл на канале 'Доверие' (rt-mos-htlive.cdn.
# ngenix.net): этот сервер закрывает соединение сразу после каждого ответа
# на запрос плейлиста — штатное, ожидаемое поведение HLS-поллинга, а не
# сбой. -reconnect_at_eof трактует это закрытие как ошибку и гонит
# ffmpeg переподключаться заново — плейлист успевает обновиться до нового
# состояния, ffmpeg тут же снова "доходит до EOF" и реконнектится опять,
# до бесконечности, ни разу не добравшись до реальной загрузки сегмента
# ("Recording finished without creating a usable video file"). Без этого
# флага тот же канал пишется мгновенно и чисто. Остальные три флага ниже
# (-reconnect_on_network_error, -reconnect_on_http_error, -rw_timeout)
# ловят настоящие сетевые обрывы/зависания без этого побочного эффекта —
# их одних уже должно быть достаточно и для случая 'Звезда'.
RECONNECT_OPTS = [
    '-reconnect', '1',
